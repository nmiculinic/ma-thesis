...
Better approximation, however, is between those two extremes, greedy and exhaustive search. Beam search decoding~\cite{graves_decode}. At each time step, we're only looking at top $N$ paths up until this point. Parameter $N$ is called beam width. From time $t$ to $t+1$ we have dynamic programming transitions, where we look at all possible path continuations and keep only the best $N$ ones until reached the end. Then we output the best one as our decoded path, apply post-processing (merge repeated if configure in that mode, drop blanks) and we're done. It's visually depicted in figure~\ref{fg:beam_search}.

\begin{figure}
    \begin{center}
        \includegraphics[width=0.8\textwidth]{beam_search}
        \caption{Beam search with beam width=3. At each time step we're remembering top beam width paths, and expand from those to generate top beam width path in next time step.}
        \label{fg:beam_search}
    \end{center}
\end{figure}
...
The config file definiton is in figure~\ref{fg:train_cfg_py} and example config file for training is in figure~\ref{fg:train_cfg_yml}. The source code can be found in module \texttt{mincall.train.\_train.py} and we recommend reading it for extended details.
...
Due to resource constraints both time and compute, I'm unable to run my own testing on my data with each basecaller. Thus I'm reusing Ryan's Wick's basecalled files for my analysis. He used Klebsiella pneumoniae bacteria.
...
\begin{figure}
\input{./fig_gen/all_consensus_report.tex}
\caption{Consensus report among the tested basecallers. All non-mincall ones are taken from~\cite{rwick_basecalling_cmp}. Mincall b50 was performed on test data subset due to time consideration.}
...
\begin{abstract}
The MinION device by Oxford Nanopore Technologies is the first portable DNA sequencing device. 
Main advantages include producing longer reads than competing technologies and real-time data analysis making it suitable for a wide array of possible applications. 
Although long reads of up to 882 000 bp can be achieved, this comes at a cost - an error rate of 10\% or higher.
The goal of this thesis is to explore novel basecaller training technique using multi-task training and autoencoder loss as a secondary task to improve performance of basecalling.
The model has been trained on R9.4 E.Coli dataset and has been compared with contemporary solutions on Klebsiella pneumoniae dataset. The complete source code is available on \url{https://github.com/nmiculinic/minion-basecaller/}.
...
\begin{sazetak}
    Uređaj MinION od firme Oxford Nanopore Technologies je prvi portabilni DNA sekvencer. 
    Glavne prednosti uključuju dulje sekvence nego konkurentske tehnologije i analitiku u stvarnom vremenu. 
    Iako duge sekvence do 882 000 baznih parova mogu biti postignute, to dolazi uz cijenu preciznosti of 10\% ili vise. 
    Cilj ovog diplomskog rada je istražiti novu metodu treniranja sekvencora koristeći vise-zadačni trening i autoenkoder kao sekundarni zadatak u svrhu unaprijeđena točnosti primarnog, sekvencioniranja. 
    Model je istreniran na R9.4 E.Coli podacima i uspoređen je s ostalim rješenjima na Klebsiella pneumoniae podacima. 
    Kompletni se izvorni kod nalazi na \url{https://github.com/nmiculinic/minion-basecaller/}.
